...
class chemspecAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('chemspec/chemspec_algorithm.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'chemspec','page':'algorithm'})
        ...
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'chemspec', 
                'model_attributes':'chemspec Algorithms', 
                'text_paragraph':x})
...
app = webapp.WSGIApplication([('/.*', chemspecAlgorithmPage)], debug=True)
