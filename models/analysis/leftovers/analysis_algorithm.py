...
class analysisAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('analysis/analysis_algorithm.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'analysis','page':'algorithm'})
        ...
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'analysis', 
                'model_attributes':'analysis Algorithms', 
                'text_paragraph':x})
...
app = webapp.WSGIApplication([('/.*', analysisAlgorithmPage)], debug=True)
