...
class pchempropAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('pchemprop/pchemprop_history.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'pchemprop','page':'history'})
        ...
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'pchemprop', 
                'model_attributes':'pchemprop User History', 
                'text_paragraph':x})
