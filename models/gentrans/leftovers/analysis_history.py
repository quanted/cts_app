...
class gentransAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('gentrans/gentrans_history.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'gentrans','page':'history'})
        ...
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'gentrans', 
                'model_attributes':'gentrans User History', 
                'text_paragraph':x})
