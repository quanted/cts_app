...
class gentransBatchInputPage(webapp.RequestHandler):
    def get(self):
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'gentrans','page':'batchinput'})
        ...
        html = html + template.render(templatepath + '04uberbatchinput.html', {
                    'model':'gentrans',
                    'model_attributes':'gentrans Batch Input'})
...
app = webapp.WSGIApplication([('/.*', gentransBatchInputPage)], debug=True)
