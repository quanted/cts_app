...
start celery -A celery_cts worker -Q chemaxon --loglevel=info --concurrency=2 -n chemaxon_worker

start celery -A celery_cts worker -Q epi --loglevel=info --concurrency=8 -n epi_worker
rem start celery -A celery_cts worker -Q sparc --loglevel=info --concurrency=1 -n sparc_worker
