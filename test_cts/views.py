...
def simple_proxy(request, path):
  ...
  logging.warning("Requested Url: {}".format(TEST_URL))
  ...
  try:
    ...
    return HttpResponse(async_data, status=async_request.status_code, content_type=async_request.headers['content-type'])
