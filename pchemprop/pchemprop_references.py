...
class pchempropReferencesPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('pchemprop/pchemprop_references.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'pchemprop','page':'references'})
        ...
        html = html + template.render(templatepath + '04uberreferences_start.html', {
                'model':'pchemprop', 
                'model_attributes':'pchemprop References', 
                'text_paragraph':x})
...
app = webapp.WSGIApplication([('/.*', pchempropReferencesPage)], debug=True)
