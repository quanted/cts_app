...
class pchempropBatchInputPage(webapp.RequestHandler):
    def get(self):
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'pchemprop','page':'batchinput'})
        ...
        html = html + template.render(templatepath + '04uberbatchinput.html', {
                    'model':'pchemprop',
                    'model_attributes':'pchemprop Batch Input'})
...
app = webapp.WSGIApplication([('/.*', pchempropBatchInputPage)], debug=True)
