...
class analysisDescriptionPage(webapp.RequestHandler):
    def get(self):
        text_file = open('analysis/analysis_description.txt','r')
        x = text_file.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'analysis','page':'description'})
...
app = webapp.WSGIApplication([('/.*', analysisDescriptionPage)], debug=True)
