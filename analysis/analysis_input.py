...
class analysisInputPage(webapp.RequestHandler):
    def get(self):
        ...
        html = html + template.render(templatepath + '04uberinput_start_tabbed.html', {
                'model':'analysis', 
                'model_attributes':'analysis Inputs'})
        html = html + str(analysis_parameters.analysisInp())
...
app = webapp.WSGIApplication([('/.*', analysisInputPage)], debug=True)
