...
class analysisAlgorithmPage(webapp.RequestHandler):
    def get(self):
        text_file1 = open('analysis/analysis_history.txt','r')
        x = text_file1.read()
        templatepath = os.path.dirname(__file__) + '/../templates/'
        html = template.render(templatepath + '01cts_uberheader.html', {'title'})
        html = html + template.render(templatepath + '02cts_uberintroblock_wmodellinks.html', {'model':'analysis','page':'history'})
        ...
        html = html + template.render(templatepath + '04uberalgorithm_start.html', {
                'model':'analysis', 
                'model_attributes':'analysis User History', 
                'text_paragraph':x})
